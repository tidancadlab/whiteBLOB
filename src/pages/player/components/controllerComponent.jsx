...
export const RenderSeekBar = ({ playerData, handleSeek }) => {
  // const [timerLeft, setTimerLeft] = useState({ left: 0, time: 0 });
  const { timeRanges, duration, currentTime } = playerData;
  const seekPercentCompleted = currentTime / duration || 0;
    ...
    <div className="absolute bottom-12 w-full px-4">
      ...
      <div className="relative h-2 w-full">
        <input
          ...
          min={0}
          max={1000}
          value={seekPercentCompleted * 1000 || 0}
          onChange={handleSeek}
        />
        <div style={{ width: `${seekPercentCompleted * 100}%` }} className="absolute top-1/2 z-10 h-1 -translate-y-1/2 rounded-lg bg-red-500"></div>
        <div className="absolute top-1/2 h-1 w-full -translate-y-1/2 rounded-md bg-white/10">
          {new Array(timeRanges?.video.length).fill(0).map((v, i) => {
            const left = timeRanges?.video.start(i);
            const width = timeRanges?.video.end(i);
            return (
              <div
              key={"buffer-bar-" + i}
                style={{ width: `${((width - left) / duration) * 100}%`, left: `${(left / duration) * 100}%` }}
                className="absolute h-full bg-white/50"></div>
            );
          })}
        </div>
      </div>
...
export const RenderPlayerSettingCard = ({ showLevels, setShowLevels, audioTrack, hls, hlsData, setHlsData }) => {
  ...
  const handleBlur = (e) => {
    if (e.relatedTarget?.id === 'gear') return;
    setShowLevels(false);
  };
